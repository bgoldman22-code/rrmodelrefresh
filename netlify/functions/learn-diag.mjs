...
function err(message, code=500){ return new Response(JSON.stringify({ ok:false, error: message }), { status: code, headers: { "content-type": "application/json" }}); }
...
export default async (req) => {
  try{
    ...
    const models = [
      { key:"mlb_hr",        label:"MLB HR",        store:["mlb-learning","mlb_hr-learning"],        func:"mlb-daily-learn" },
    ...
    async function funcReachable(name){
      try{
        const r = await fetch(`${origin}/.netlify/functions/${name}?dry=1`);
        if(!r.ok) return { reachable:false, code:r.status };
        const j = await r.json().catch(()=> ({}));
        return { reachable:true, body:j };
      }catch(e){
    ...
    for(const m of models){
      const item = { label:m.label, status:"yellow", detail:"", picksToday:false, lastRun:null, samples:0, daysLearned:0, func:{} };
