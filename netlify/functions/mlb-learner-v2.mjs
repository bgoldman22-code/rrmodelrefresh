export default async (req) => {
  try{
    ...
    if(dry){
      return new Response(JSON.stringify({ ok:true, dry:true, date, version:"v2-shim" }), { headers:{ "content-type":"application/json" } });
    }
    // safe proof-of-life (no network, no imports)
    return new Response(JSON.stringify({ ok:true, date, samplesAdded:1, daysAdded:1, version:"v2-shim" }), { headers:{ "content-type":"application/json" } });
  }catch(e){
